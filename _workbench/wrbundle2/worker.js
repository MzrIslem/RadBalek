...
async function handleWeather(env, geo) {
  ...
  const url = `https://api.open-meteo.com/v1/forecast?latitude=${lats}&longitude=${lons}&current=temperature_2m,relative_humidity_2m,wind_speed_10m,apparent_temperature&daily=apparent_temperature_max&forecast_days=3&timezone=UTC`;
...
var worker_default = {
  ...
  async fetch(req, env, ctx) {
    ...
    const CACHE_TTL = {
      "/v1/alerts.json": 120,
      "/v1/reports.json": 120,
      // >=86.4s = under the 1k/day list quota alone
      "/v1/reports.csv": 300,
      // 1 list + up to 1000 gets — must be cached
      "/v1/history.json": 300,
