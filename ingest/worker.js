...
import { errText, logSwallowed } from "./src/log.js";

export default {
  async scheduled(_event, env, ctx) {
    // A rejected waitUntil promise is reported as an unhandled rejection with no
    // context; name the cycle that died so the log says which one.
    ctx.waitUntil(refresh(env, true).catch((err) => logSwallowed("cron:refresh", err)));
  },

  // An uncaught throw anywhere below returns Cloudflare's opaque 1101 page —
  // no CORS headers (so the app reports "offline" instead of the real status)
  // and no log line. Route every unexpected failure through one handler that
  // logs it and answers with a CORS-safe JSON 500.
  async fetch(req, env, ctx) {
    try {
      return await handleFetch(req, env, ctx);
    } catch (err) {
      console.error(`[rb] unhandled ${req.method} ${new URL(req.url).pathname}: ${errText(err)}`);
      return new Response(JSON.stringify({ error: "internal", detail: errText(err) }), {
        status: 500,
...
async function handleFetch(req, env, ctx) {
  ...
  if (path === "/v1/alerts.json") {
    ...
    if (!body) {
      try {
        ...
        console.error(`[rb] on-demand refresh failed: ${errText(err)}`);
        return new Response("refresh failed: " + errText(err), { status: 500, headers: corsHeaders() });
      }
    }
  ...
  if (path === "/v1/admin/refresh" && req.method === "POST") {
    ...
    try {
      ...
      console.error(`[rb] admin refresh failed: ${errText(err)}`);
      return new Response(JSON.stringify({ error: errText(err) }), {
        status: 500,
        headers: corsHeaders({ "content-type": "application/json; charset=utf-8" }),
      });
    }
  ...
  if (path === "/v1/fwi.png" || path === "/v1/burnt.png") {
    ...
    if (!isPng(img)) {
      ...
      try {
        ...
        logSwallowed(`kv:put ${kvKey}`, err); // served anyway, just not cached
      }
    }
  ...
  if (path === "/v1/history.json") {
    ...
    if (!body) {
      ...
      for (const k of listed.keys) {
        ...
        if (!raw) continue;
        // One truncated record used to throw out of the handler and 500 the
        // whole endpoint; skip it and keep the other 167 hours.
        try {
          out.push(JSON.parse(raw));
        } catch (err) {
          logSwallowed(`history:parse ${k.name}`, err);
        }
      }
...
async function reconcileFires(env, snap) {
  try {
    ...
    try {
      ...
      logSwallowed("firms:cache read", err);
    }
    const cacheUsable =
      cached &&
    ...
    if (!snap.stats.firmsSkipped && !untrustedZero) {
      ...
      try {
        ...
        // The fire map keeps working this cycle, but the fallback for the next
        // skipped cycle is now stale — record it in the snapshot's errors[] so
        // the watchdog and /v1/admin/overview can see it.
        snap.errors.push({ source: "firms-cache", error: errText(err) });
        logSwallowed("firms:cache write", err);
      }
      return;
    }
    ...
    // Never breaks a snapshot — but a silently broken fire-map fallback looked
    // exactly like "no fires in Algeria".
    snap.errors.push({ source: "firms-reconcile", error: errText(err) });
    logSwallowed("firms:reconcile", err);
  }
}
...
async function refresh(env, doPush = false) {
  ...
  try {
    ...
    // The push path below still runs, but the app now serves the PREVIOUS
    // snapshot — which the dead-man switch reads as a dead cron. Recording the
    // failure is what tells the watchdog the difference.
    snap.errors.push({ source: "kv-latest", error: errText(err) });
    logSwallowed("kv:put latest", err);
  }
  ...
  try {
    ...
    if (eventful || now.getUTCMinutes() < 10) {
      try {
        ...
        logSwallowed("kv:put push:last", err);
      }
    }
    await watchPipeline(env, snap, pushSummary);
  } catch (err) {
    console.error(`[rb] push cycle failed: ${errText(err)}`);
    try {
      await env.EWS_KV.put("push:last", JSON.stringify({ at: snap.generatedAt, fatal: errText(err) }));
    } catch (kvErr) {
      logSwallowed("kv:put push:last (fatal)", kvErr);
    }
    await watchPipeline(env, snap, { fatal: errText(err) });
  }
  ...
  try {
    if (now.getUTCMinutes() < 10) {
      ...
      try {
        ...
        logSwallowed("kv:put history entry", err);
      }
      ...
      try {
        ...
        logSwallowed("kv:put history:doc", err);
      }
    }
  } catch (err) {
    logSwallowed("history block", err); // analytics only — the cycle stands
  }
  return snap;
}
